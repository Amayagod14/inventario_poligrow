...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($marca) || empty($modelo) || empty($serial) || empty($placa_activos) || empty($cedula) || empty($estado) || empty($fecha_entrega) || empty($fecha_compra)) {
        $_SESSION['error'] = 'Por favor, completa todos los campos requeridos.';
        header('Location: ../vistas/agregar_impresoras.php');
        exit();
    }

    // Insertar en la tabla impresoras
    $stmt = $pdo->prepare("INSERT INTO impresoras (marca, modelo, serial, placa_activos, cedula, estado, fecha_entrega, fecha_compra, observaciones) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)");
    if ($stmt->execute([$marca, $modelo, $serial, $placa_activos, $cedula, $estado, $fecha_entrega, $fecha_compra, $observaciones])) {
        // Mensaje de éxito
